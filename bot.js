...
bot.on('message', message => {
    ...
    if(userData[sender.id + message.guild.id].messagesSent === 5){
    	if(!message.member.roles.find("name", "Admin"||"Test")){
    		bot.channels.get('389161550797078538').send(message.author + ' has sent at least ' + userData[sender.id + message.guild.id].messagesSent + ' messages to this server .');
    	}
    }

    if (msg.startsWith(prefix + 'SETUSERINFO')) {
...
///////////////////////////////////////////////
    ...
    if (msg.startsWith(prefix + 'SETUSERINFO')) { // This time we have to use startsWith, since we will be adding a number to the end of the command.
        ...
        async function SETUSERINFO() {
            ...
            if (isNaN(args[1])) {
                ...
                message.channel.send('Please use a number as your arguments. \n Usage: ' + prefix + 'SETUSERINFO <amount>'); //\n means new line.
            ...
            userlist.forEach(function(user){
            	...
            	userData[user.id + message.guild.id].messagesSent = newMsgSent;
            	console.log(user.id + 's message count set to ' + args[1]); // Lets post into console how many messages a user now has
            })
        }
	...
	//If message is in staff or admin channels, ignore.
	if(!(message.channel.id == '389639792960339979' || message.channel.id == '389161550797078538')){
